...
DESCRIPTIONS = {
    ...
    "doc_type": "Dateityp des Belegs",
    "original_filename": "Ursprünglicher Dateiname beim Hochladen",
    "doc_text": "Textebene der PDF (für die Suche)",
    "suggestion_json": "Beim Hochladen aus dem Beleg gelesene Werte (JSON)",
...
def create_export(settings: Settings, year: str | None = None) -> Path:
    ...
    try:
        ...
        backup._record(settings, "export", False, f"{label}: {e}")
        raise
    backup._record(settings, "export", True, target.name)
    return target
...
def _create_export(settings: Settings, year: str | None) -> Path:
    ...
    problems = [(label, p) for _arc, path, sha, label in documents
